...
class AD7606_SPI:
    ...
    def ADCread(self):
        ...
        #while self.io.read(self.p_busy) == 1:
            #continue
        _c, _d = self.io.spi_read(self.adc,16) # two bytes per each of the 6 channels. _c is number of bytes read back and _d is a bytearray of said bytes.

        if self.returnRaw:
            return _d
        else:
            _channel1 = [self.transferFunction(_d[0] << 8 | _d[1]), _d[0] << 8 | _d[1]]
            _channel2 = [self.transferFunction(_d[2] << 8 | _d[3]), _d[2] << 8 | _d[3]]
            _channel3 = [self.transferFunction(_d[4] << 8 | _d[5]), _d[4] << 8 | _d[5]]
            _channel4 = [self.transferFunction(_d[6] << 8 | _d[7]), _d[6] << 8 | _d[7]]
            _channel5 = [self.transferFunction(_d[8] << 8 | _d[9]), _d[8] << 8 | _d[9]]
            _channel6 = [self.transferFunction(_d[10] << 8 | _d[11]), _d[10] << 8 | _d[11]]
            _channel7 = [self.transferFunction(_d[12] << 8 | _d[13]), _d[12] << 8 | _d[13]]
            _channel8 = [self.transferFunction(_d[14] << 8 | _d[15]), _d[14] << 8 | _d[15]]

            return [_channel1,_channel2,_channel3,_channel4,_channel5,_channel6,_channel7,_channel8]
